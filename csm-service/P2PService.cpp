...
int P2PService::init()
{
    ...
    m_serviceConfig->sessionDestroyer()->setHandler(
        [this](const int fd, const int flag)
        {
            ...
            // 设置本节点到该离线节点的距离为不可达
            LOG->write(utilities::LogType::Log_Info, FILE_INFO, "set neighbour unreachable, node id: ", p2pSession->nodeId());
            m_serviceConfig->distanceVector()->setNightbourUnreachable(p2pSession->nodeId());
            ...
            if (ServiceStartType::Node == m_serviceConfig->serviceStartType())
            {
                ...
                disconnectClient(p2pSession->peerHostEndPointInfo(), p2pSession->nodeId(), p2pSession->handshakeUuid(), flag, fd);
            }
    ...
    m_serviceConfig->sessionModuleDataProcessor()->registerPacketHandler(
        ...
        [this](const int fd, PacketHeader::Ptr header, PayloadBase::Ptr payload) -> int
        {
            ...
            {
                LOG->write(utilities::LogType::Log_Info, FILE_INFO, "online timestamp: ", p2pSession->clientOnlineTimestamp(),
...
int P2PService::initServer()
{
    ...
    m_serviceConfig->acceptor()->setNewClientCallback(
        [this](const int fd, P2PSession::Ptr p2pSession)
        {
            // 将fd添加到P2PSession派发器中，由P2PSessionManager决定P2PSession由哪个SlaveReactor管理
            m_serviceConfig->sessionDispatcher()->addSession(
                fd,
                [this, fd, p2pSession](const std::size_t slaveReactorIndex)
                {
                    LOG->write(utilities::LogType::Log_Info, FILE_INFO,
                               "will dispatch P2PSession to SlaveReactor index: ", slaveReactorIndex, ", fd: ", fd);

                    // 将fd添加到心跳检查器中
                    if (-1 == m_serviceConfig->sessionAliveChecker()->addSession(fd))
                    {
                        LOG->write(utilities::LogType::Log_Error, FILE_INFO,
                                   "add new online client to ClientAliveChecker failed, fd: ", fd);
                        return;
                    }
                    LOG->write(utilities::LogType::Log_Info, FILE_INFO,
                               "add new online client to ClientAliveChecker successfully, fd: ", fd);

                    // 将fd和P2PSession添加到P2PSessionManager中
                    if (-1 == m_serviceConfig->p2pSessionManager()->addSession(fd, p2pSession))
                    {
                        LOG->write(utilities::LogType::Log_Info, FILE_INFO,
                                   "add new online client to P2PSessionManager failed, fd: ", fd);
                        return;
                    }
                    LOG->write(utilities::LogType::Log_Info, FILE_INFO,
                               "add new online client to P2PSessionManager successfully, fd: ", fd);

                    if (-1 == m_serviceConfig->slaveReactors()[slaveReactorIndex]->addClient(fd))
                    {
                        LOG->write(utilities::LogType::Log_Error, FILE_INFO,
                                   "add new online client to SlaveReactor failed, fd: ", fd);
                        m_serviceConfig->p2pSessionManager()->removeSession(fd);
                        return;
                    }
                    LOG->write(utilities::LogType::Log_Info, FILE_INFO, "add new online client to SlaveReactor ",
                               slaveReactorIndex, " successfully, fd: ", fd);
                });
        });
    ...
    m_serviceConfig->sessionServiceDataProcessor()->registerPacketHandler(
        ...
        [this](const int fd, PacketHeader::Ptr header, PayloadBase::Ptr packet) -> int
        {
            LOG->write(utilities::LogType::Log_Trace, FILE_INFO, "receive heartbeat, fd: ", fd);
            // 更新心跳
            ...
            if (0 != ret)
            {
                LOG->write(utilities::LogType::Log_Error, FILE_INFO, "send heartbeat reply ret: ", ret, ", fd: ", fd);
            }
...
int P2PService::initClient()
{
    ...
    m_serviceConfig->hostsConnector()->registerConnectHandler(
        [this](const int fd, P2PSession::Ptr p2pSession)
        {
            LOG->write(utilities::LogType::Log_Info, FILE_INFO, "connect ", p2pSession->peerHostEndPointInfo().host(),
                       " successfully, ready to dispatch P2PSession to SlaveReactor, fd: ", p2pSession->fd());
            ...
            m_serviceConfig->sessionDispatcher()->addSession(
                fd, [this, p2pSession, &addP2PSessionPromise](const int slaveReactorIndex)
    ...
    m_serviceConfig->sessionServiceDataProcessor()->registerPacketHandler(
        ...
        [this](const int fd, PacketHeader::Ptr header, PayloadBase::Ptr payload)
        {
            ...
            p2pSession->setNodeId(id);
            p2pSession->setHandshakeUuid(payloadClientInfo->handshakeUuid());
            p2pSession->setNodeOnlineTimestamp(utilities::TimeTools::getCurrentTimestamp());
            ...
            LOG->write(utilities::LogType::Log_Info, FILE_INFO, "recv ClientInfo, fd: ", fd, ", id: ", id,
            ...
            else
            {
                ...
                {
                    LOG->write(utilities::LogType::Log_Info, FILE_INFO,
                               "add host id info to HostsInfoManager successfully, fd: ", fd, ", id: ", id);
                }
            }
            ...
            {
                LOG->write(utilities::LogType::Log_Error, FILE_INFO, "send ClientInfoReply payload failed", ", fd: ", fd,
                           ", id: ", id);
                return -1;
            }
        ...
        [this](const int fd, PacketHeader::Ptr header, PayloadBase::Ptr payload) -> int
        {
            ...
            if (0 != result)
            {
                ...
                if (-3 == result || -4 == result)
                {
                    ...
                    LOG->write(utilities::LogType::Log_Info, FILE_INFO, "get another connection fd: ", anotherConnectionFd,
                               ", fd: ", fd);
                    ...
                    LOG->write(utilities::LogType::Log_Info, FILE_INFO, "another connection fd: ", anotherConnectionFd,
                               ", peer host: ", anotherP2PSession->peerHostEndPointInfo().host(), ", fd: ", fd);
                }
                LOG->write(utilities::LogType::Log_Info, FILE_INFO, "add tcp session to destoryer, fd: ", fd);
            ...
            else
            {
                ...
                LOG->write(utilities::LogType::Log_Info, FILE_INFO,
                           "set host id successfully, host: ", peerHostEndPointInfo.host(), ", id: ", id, ", fd: ", fd);
                ...
                {
                    LOG->write(utilities::LogType::Log_Error, FILE_INFO,
                               "set host connected failed, host: ", peerHostEndPointInfo.host(), ", fd: ", fd);
                    return -1;
                }
                LOG->write(utilities::LogType::Log_Info, FILE_INFO,
                           "set host connected successfully, host: ", peerHostEndPointInfo.host(), ", fd: ", fd);
                ...
                m_serviceConfig->distanceVector()->addNeighbourNode(id);
            }
    ...
    m_serviceConfig->sessionServiceDataProcessor()->registerPacketHandler(
        ...
        [this](const int fd, PacketHeader::Ptr header, PayloadBase::Ptr payload) -> int
        {
            LOG->write(utilities::LogType::Log_Trace, FILE_INFO, "receive heartbeat reply, fd: ", fd);
            m_serviceConfig->sessionAliveChecker()->refreshSessionLastRecvTime(fd);
...
int P2PService::disconnectClient(const HostEndPointInfo& hostEndPointInfo, const std::string& id, const std::string& uuid,
                                 const int flag, int fd)
{
    LOG->write(utilities::LogType::Log_Info, FILE_INFO, "id: ", id, ", flag: ", flag,
               ", hostEndPointInfo: ", hostEndPointInfo.host(), ", fd: ", fd);
    ...
    if (0 == flag || -1 == flag)
    {
        ...
        {
            LOG->write(utilities::LogType::Log_Info, FILE_INFO, "remove host id info failed, id: ", id, ", fd: ", fd);
        }
        else
        {
            LOG->write(utilities::LogType::Log_Info, FILE_INFO, "remove host id info successfully, id: ", id, ", fd: ", fd);
        }

        if (-1 == m_serviceConfig->hostsInfoManager()->setHostNotConnected(hostEndPointInfo))
        {
            LOG->write(utilities::LogType::Log_Info, FILE_INFO, "set host not connect failed: ", hostEndPointInfo.host(), ", fd: ", fd);
        }
        else
        {
            LOG->write(utilities::LogType::Log_Info, FILE_INFO, "set host not connect successfully: ", hostEndPointInfo.host(), ", fd: ", fd);
        }
    }
    else if (-2 == flag)
    {
