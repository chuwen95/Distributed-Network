...
#include "csm-service/routing_algorithm/distance_vector/DistanceVectorServiceFactory.h"
...
std::unique_ptr<P2PService> P2PServiceFactory::create()
{
    ...
    // 路由选择算法
    std::unique_ptr<DistanceVectorServiceInterface> distanceVector = createDistanceVector();
...
std::unique_ptr<DistanceVectorServiceInterface> P2PServiceFactory::createDistanceVector()
{
    return DistanceVectorServiceFactory::create(m_nodeConfig->nodeId(), m_nodeConfig->clusterServerIds());
}