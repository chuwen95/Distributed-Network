...
int PacketHeader::setModuleId(protocol::ModuleID moduleId)
{
    m_moduleId = moduleId;
    return 0;
}

csm::protocol::ModuleID PacketHeader::moduleId() const
{
    return m_moduleId;
}
...
int PacketHeader::encode(char* buffer, const std::size_t length)
{
    ...
    *(reinterpret_cast<std::int32_t*>(buffer + sizeof(m_magic) + sizeof(m_packetType))) =
        htonl(static_cast<std::int32_t>(m_moduleId));
    *(reinterpret_cast<std::uint32_t*>(buffer + sizeof(m_magic) + sizeof(m_packetType) + sizeof(m_moduleId))) =
        htonl(m_payloadLength);
...
int PacketHeader::decode(const char* buffer, const std::size_t length)
{
    ...
    m_moduleId = static_cast<protocol::ModuleID>(
        ntohl(*(reinterpret_cast<std::int32_t*>(const_cast<char*>(buffer + sizeof(m_magic) + sizeof(m_packetType))))));
    m_payloadLength = ntohl(*(reinterpret_cast<std::uint32_t*>(
        const_cast<char*>(buffer + sizeof(m_magic) + sizeof(m_packetType) + sizeof(m_moduleId)))));
