...
namespace components
{
    ...
    int Socket::create()
    {
        ...
        if (-1 == fd)
        {
            Singleton<Logger>::instance()->write(components::LogType::Log_Error, "create socket failed, ", "errno: ", errno, ", " ,
                                                 strerror(errno));
            return -1;
        }
    ...
    int Socket::setNonBlock(const int fd)
    {
        ...
        if(-1 == flags)
        {
            Singleton<Logger>::instance()->write(components::LogType::Log_Error, "get socket flags failed, ", "errno: ", errno, ", " ,
                                                 strerror(errno));
            return -1;
        }
        if(-1 == fcntl(fd, F_SETFL, flags | O_NONBLOCK))
        {
            Singleton<Logger>::instance()->write(components::LogType::Log_Error, "set socket nonblock failed, ", "errno: ", errno, ", " ,
                                                 strerror(errno));
            return -1;
        }
        return 0;
    ...
    int Socket::bind(const int fd, const std::string_view ip, const unsigned int port)
    {
        ...
        {
            Singleton<Logger>::instance()->write(components::LogType::Log_Error, "inet_pton ip address failed, ", "errno: ", errno, ", " ,
                                                 strerror(errno));
            return -1;
        }
        ...
        {
            Singleton<Logger>::instance()->write(components::LogType::Log_Error, "bind ip and port failed, ", "errno: ", errno, ", " ,
                                                 strerror(errno));
            return -1;
        }
        return 0;
    ...
    int Socket::listen(const int fd, const std::size_t conNum)
    {
        ...
        {
            Singleton<Logger>::instance()->write(components::LogType::Log_Error, "listen failed, ", "errno: ", errno, ", " ,
                                                 strerror(errno));
        }
        return 0;
    }
