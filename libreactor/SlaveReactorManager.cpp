...
namespace server
{
        ...
        const auto expression = [this]()
        {
            ...
            while(true == m_tcpSessionsQueue.dequeue(tcpSession))
            {
                ...
                components::Singleton<components::Logger>::instance()->write(components::LogType::Log_Info, FILE_INFO,
                                                                             "dispatch TcpSession to slave reactor, index: ", m_slaveReactorIndexWhichHasLeastFd.load());
                m_slaveReactors[m_slaveReactorIndexWhichHasLeastFd]->addClient(std::shared_ptr<TcpSession>(tcpSession));

                ++s_refreshTime;
                if(0 == s_refreshTime % 50)
                {
    ...
    void SlaveReactorManager::registerDisconnectHandler(std::function<void(const int)> disconnectHandler)
    {
        for(auto& slaveReactor : m_slaveReactors)
        {
            slaveReactor->registerDisconnectHandler(disconnectHandler);
        }
    }
