...
namespace service
{
    ...
    public:
        ...
        {
            std::vector<std::string> values = components::string_tools::split(host, ':');
            if(2 == values.size())
            {
                m_ip = values[0];
                m_port = components::string_tools::convertFromString<unsigned short>(values[1]);
            }
        }
        ~HostEndPointInfo() = default;
        ...
        std::string host() const { return m_ip + ":" + std::to_string(m_port); }
