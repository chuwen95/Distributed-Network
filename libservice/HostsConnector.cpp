...
namespace service
{
    ...
    int HostsConnector::init(HostsInfoManager::Ptr hostsInfoManager)
    {
        ...
        const auto expression = [this]()
        {
            ...
            else
            {
                for (auto iter = m_connectingHosts.begin(); iter != m_connectingHosts.end(); )
                {
                    ...
                    if (FD_ISSET(fd, &wset))
                    {
                        ...
                        {
                            components::Singleton<components::Logger>::instance()->write(components::LogType::Log_Info, FILE_INFO,
                                                                                         "connect to ", iter->first.host()," failed, error: ", error);
                            components::Socket::close(fd);
                            iter = m_connectingHosts.erase(iter);
                            continue;
                        }
                        ...
                        components::Singleton<components::Logger>::instance()->write(components::LogType::Log_Info, FILE_INFO,
                                                                                     "connect to ", iter->first.host()," successfully, fd is set");
                        if(nullptr != m_connectHandler)
                        {
                            components::Singleton<components::Logger>::instance()->write(components::LogType::Log_Info, FILE_INFO,
                            ...
                            components::Singleton<components::Logger>::instance()->write(components::LogType::Log_Info, FILE_INFO,
                                                                                         "connect to ", iter->first.host()," callback TcpSession finish");
                        }
            ...
            components::Singleton<components::Logger>::instance()->write(components::LogType::Log_Info, FILE_INFO,
                                                                         "online node size: ", m_hostsInfoManager->onlineClientSize());
