...
namespace service
{
    ...
    int HostsInfoManager::init(const std::string& configFile)
    {
        ...
        if(false == nodeFile.is_open())
        {
            components::Singleton<components::Logger>::instance()->write(components::LogType::Log_Error, FILE_INFO,
                                                                         "open nodes config file failed: ", configFile);
            return -1;
        }
        ...
        components::Singleton<components::Logger>::instance()->write(components::LogType::Log_Debug, FILE_INFO,
                                                                     "nodes config file content: ", jsonContent);
        ...
        for(auto iter = nodesValue.begin(); iter != nodesValue.end(); ++iter)
        {
            ...
            components::Singleton<components::Logger>::instance()->write(components::LogType::Log_Info, FILE_INFO,
                                                                         "hosts info: ", host);
            std::vector<std::string> ipPort = components::string_tools::split(host, ':');
            if(2 != ipPort.size())
            {
                components::Singleton<components::Logger>::instance()->write(components::LogType::Log_Error, FILE_INFO,
                                                                             "parse hosts ip and port failed: ", host);
                return -1;
            }

            std::string ip = ipPort[0];
            unsigned short port = components::string_tools::convertFromString<unsigned short>(ipPort[1]);
    ...
    int HostsInfoManager::removeHostIdInfo(const std::string &id, const std::string& uuid)
    {
        ...
        {
            components::Singleton<components::Logger>::instance()->write(components::LogType::Log_Error, FILE_INFO,
                                                                         "host id not exist, id: ", id);
            return -1;
        }
        if(iter->second.second != uuid)
        {
            components::Singleton<components::Logger>::instance()->write(components::LogType::Log_Error, FILE_INFO,
                                                                         "uuid not match, id: ", id);
            return -1;
        }
