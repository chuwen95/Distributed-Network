...
namespace service
{
    ...
    int PacketHeader::decode(const char *buffer, const std::size_t length)
    {
        ...
        m_moduleId = ntohl(*(reinterpret_cast<std::int32_t*>(const_cast<char*>(buffer + sizeof(m_magic) + sizeof(m_packetType)))));
        m_payloadLength = ntohl(*(reinterpret_cast<std::uint32_t*>(const_cast<char*>(buffer + sizeof(m_magic) + sizeof(m_packetType) + sizeof(m_moduleId)))));
