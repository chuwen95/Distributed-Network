...
12. 回调函数进行解payloadData为请求包操作（调用PacketFactory的createPacket方法，传入packetType和payloadData），创建出请求包后，构造一个空的回应包，请求包继承于PacketBase，回应包继承于PacketReplyBase。

13. 有了请求包类型，请求包，准备好的空回应包，调用PacketProcessor的process方法，传入三者作为参数，PacketProcess会根据包类型创建对应的处理器。即业务层只需要实现好处理器，定义好继承于PacketBase和PacketReplyBase的请求回应包就可以了。

14. 处理完后，对回应包进行编码，将数据写入到发送环形缓冲区中，SlaveReactor会自动发送回应数据。

15. 11-13所说的回调函数会放入ThreadPool中，ThreadPool存在于TcpService，对业务层是无感的，业务层是被packetprocess命名空间包含的部分。

16. ThreadPool里面是一个MPMC的无锁队列。

17. 对于ClientInfo和ClientInfoReply包，是立即处理的，不会放到线程池中处理，这是socket连接上后立即发送的请求和回应包，包里面的信息用于表明节点身份和进行握手，对节点互相连接共享一条连接有关键作用，后面有专门的文档描述实现机制。
